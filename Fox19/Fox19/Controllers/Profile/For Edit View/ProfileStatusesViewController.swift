...
class ProfileStatusesViewController: UIViewController {
    ...
    @objc private func updateUser() {
        ...
        userToUpdate.isGamer = gamerStatusSwitch.isOn
        userToUpdate.isTrainer = trainerStatusSwitch.isOn
        networkManager.updateUser(user: userToUpdate) { [weak self] result in
            guard let self = self else { return }
            switch result {
            case .success(_):
                self.hide()
    ...
    public func configureSelector(user: User) {
        guard let isGamer = user.isGamer,
              let isTrainer = user.isTrainer else {
            return }
